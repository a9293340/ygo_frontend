...
const removeDeck = async () => {
  const target_id = deckList.value.find(el => el.admin_id === deck_check_id.value);
  if (target_id) {
    try {
      const data = await callApi<DeckDeleteType>({ _id: target_id._id }, 'deck', 'delete', false);
    } catch (error) {}
...
<template>
  <div class="deck-list">
    <div class="search-bar">
      ...
      <el-button type="primary" @click="getList({ limit: listQuery.limit, page: 0 })">{{
        t('card.search')
      }}</el-button>
    </div>
    <div class="list-container">
      <div class="card-info-box">
        ...
        >
          ...
          <div class="sm-item">
            <el-button type="warning" @click="openDialog('e', i)">{{ t('deck.edit') }}</el-button>
          </div>
          <div class="sm-item">
            <el-button type="danger" @click="openDialog('r', i)">{{ t('deck.remove') }}</el-button>
          </div>
          <div class="sm-item">
  ...
  <el-dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    <el-button style="width: 100px" type="danger" v-if="removeVisible" @click="removeDeck">{{
      t('deck.remove')
    }}</el-button>
    <el-button style="width: 100px" type="warning" v-if="editVisible">{{
      t('deck.edit')
    }}</el-button>
  </el-dialog>
</template>
