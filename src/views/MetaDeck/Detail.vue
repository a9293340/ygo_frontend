<template>
  <div class="meta-deck-detail">
    <div class="article-container">
      <img class="top-img" :src="`/api/card-image/article/${article.photo}`" alt="">
      <h1>{{ article.title }}</h1>
      <div class="info">
        <div>{{ `${$t("article.author")}：${article.author_name}` }}</div>
        <div>{{ `${$t("article.date")}：${formatDateString(article.publish_date)}` }}</div>
      </div>
...
const article = ref<BaseArticle | NormalArticle>({
  ...
  content: "<p style='color:red;'><strong>3/4-3/12 上位卡表抄牌區</strong></p><p><strong>4月禁卡表被劍神官洗回去，只好繼續抄本週環境的牌</strong></p><p><strong>本週的60張魔救表現不錯</strong></p>",
  status: 0,
  to_top: true,
